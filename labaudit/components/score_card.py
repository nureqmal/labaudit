...
def render_score_gauge(report: AuditReadinessReport) -> None:
    ...
    paper = "rgba(0,0,0,0)"
    grid  = "rgba(255,255,255,0.05)" if dark else "rgba(0,0,0,0.04)"
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "threshold": {"line": {"color": colour, "width": 2}, "thickness": 0.85, "value": score},
        },
        title={"text": f"<b style='font-family:Inter;'>Audit Readiness</b><br><span style='font-size:0.85em;color:{colour};'>{report.status_label}</span>",
               "font": {"size": 14, "family": "Inter"}},
    ))
    fig.update_layout(
        height=260, margin=dict(t=60, b=0, l=20, r=20),
...
def _pillar_card(pillar: PillarScore, icon: str) -> None:
    ...
    # colour-coded border top
    bar_pct = f"{pillar.score:.0f}%"
    issues_html = ""
    if pillar.issues:
        iss = pillar.issues[0][:55] + ("..." if len(pillar.issues[0]) > 55 else "")
        issues_html = f"<div style='font-size:0.7rem;color:{t2};margin-top:6px;line-height:1.4;'>• {iss}</div>"
    ...
    <div style="background:{bg};border:1px solid {bd};border-top:3px solid {colour};
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
            <div>
                <div style="font-size:0.7rem;font-weight:600;color:{t2};text-transform:uppercase;
                            letter-spacing:0.07em;margin-bottom:4px;">{icon} {pillar.name}</div>
                <div style="font-size:1.9rem;font-weight:700;color:{colour};line-height:1;
                            letter-spacing:-0.02em;">{pillar.score:.0f}%</div>
            </div>
        ...
        <div style="height:4px;background:{'rgba(255,255,255,0.08)' if dark else '#f1f5f9'};
            ...
            <div style="width:{bar_pct};height:100%;background:{colour};
